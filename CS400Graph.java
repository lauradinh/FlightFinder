...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Edge {
		...
		public Edge(Vertex target, double d) {
			this.target = target;
			this.weight = d;
		}
	}
	 ...
	 * @param d the weight for the edge (has to be a positive integer)
	 ...
	 */
	public boolean insertEdge(T source, T target, double d) {
		...
		for (Edge e : sourceVertex.edgesLeaving)
			if (e.target == targetVertex) {
				if (e.weight == d)
					...
					e.weight = d; // otherwise update weight of existing edge
				return true;
			}
		// otherwise add new edge to sourceVertex
		sourceVertex.edgesLeaving.add(new Edge(targetVertex, d));
		return true;
	}
